...
export default function WhyHireMeSection() {
  ...
  const animateCounters = () => {
    ...
    const animate = (now: number) => {
      ...
      setProjectsCount(Math.floor(progress * PROJECTS_TARGET));
      setSatisfaction(Math.floor(progress * SATISFACTION_TARGET));
      if (progress < 1) {
        rafRef.current = requestAnimationFrame(animate);
      } else {
        setProjectsCount(PROJECTS_TARGET);
        setSatisfaction(SATISFACTION_TARGET);
    ...
    const observer = new window.IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting && !animatingRef.current) {
          animateCounters();
        } else if (!entry.isIntersecting && (projectsCount !== 0 || satisfaction !== 0)) {
      ...
      <div className="max-w-6xl mx-auto flex flex-col lg:flex-row gap-8 lg:gap-16 items-center">
        ...
        <div className="w-full lg:w-3/5 px-4 md:px-6 lg:px-0">
          ...
          <div className="flex flex-col md:flex-row justify-start gap-8 md:gap-12 mb-10">
            <div>
              <p className={`text-4xl md:text-5xl font-bold text-gray-800 transition-all duration-300 ease-in-out ${isPopped ? 'scale-125 text-primary' : ''}`}>
                {projectsCount}+
              </p>
